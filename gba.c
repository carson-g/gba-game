...
void drawImageTransparentDMA(int row, int col, int width, int height, const u16 *image, const u16 *background) {
  int i, j;
  for (i = 0; i < height; i++) {
      ...
      DMA[3].cnt = width | DMA_ON | DMA_SOURCE_INCREMENT | DMA_DESTINATION_INCREMENT;
      for (j = 0; j < width; j++) {
        u16 pixel = image[i * width + j];
        if (pixel == BLACK) {
            setPixel(row + i, col + j, background[OFFSET(row + i, col + j, WIDTH)]);
        }
      }
  }
}
